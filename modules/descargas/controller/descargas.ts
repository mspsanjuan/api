...
// import { phantomjs } from 'phantomjs-prebuilt-that-works';
// import { phantomjs } from 'phantomjs-prebuilt-that-works';
...
export class Documento {
    ...
    static generarRegistroInsumoHTML(producto: any, template: string): any {
        return template
            ...
            .replace('<!--indicacion-->', producto.valor.indicacion !== '' ? '<b>Indicación:</b><br>' + producto.valor.indicacion : '');
    }
    ...
    static async generarInforme(registros, prestacionConceptId) {
        return new Promise(async (resolve, reject) => {
            for (let i = 0; i < registros.length; i++) {
                if (registros[i]) {
                    ...
                    if (registros[i] && registros[i].registros && registros[i].registros.length > 0) {
                        this.nivelPadre = 0;
                        this.generarInforme(registros[i].registros, prestacionConceptId);
                    }
                }
            }
            resolve(true);
    ...
    private static async generarConstanciaPuco(req) {
        this.html = fs.readFileSync(path.join(__dirname, '../../../templates/puco/constancia.html'), 'utf8');
            ...
            switch (req.params.tipo) {
                case 'pdf':
                    ...
                    await this.generarHTML(req).then(async htmlPDF => {
                        ...
                        // fs.writeFile('/home/andrrr/bin/rup.html', htmlPDF, null);
